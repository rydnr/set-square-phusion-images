...
function mongo_dump() {
  ...
  if isEmpty "${_user}"; then
    mongodump --gzip --archive="${_output}" -h ${_host} 2>&1 > /dev/null;
  else
    mongodump --gzip --archive="${_output}" -h ${_host} -u ${_user} -p ${_pass} --authenticationDatabase=${_authDb} --authenticationMechanism=${_authMechanism} 2>&1 > /dev/nullb
  fi
  _rescode=$?;
