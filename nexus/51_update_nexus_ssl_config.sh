...
function main() {
  ...
  local -i _enableSsl=${TRUE} # It's not necessary if https is handled by a reverse proxy
  if isTrue ${_enableSsl}; then
    if jettyConfAlreadyUpdated "${JETTY_HTTPS_CONFIG_FILE}"; then
      ...
      if updateKeyStorePathInJettyConf "${SSL_KEYSTORE_PATH}" "${JETTY_HTTPS_CONFIG_FILE}" "${SSL_CERTIFICATE_ALIAS}"; then
        logInfoResult SUCCESS "done"
      else
        logInfoResult FAILURE "failed"
  ...
  if isTrue ${_enableSsl}; then
    logInfo -n "Appending ssl.etc=$(command dirname ${SSL_KEYSTORE_PATH}) to ${NEXUS_SYSTEM_PROPERTIES_FILE}"
    if appendSslEtcInNexusSystemProperties "${NEXUS_SYSTEM_PROPERTIES_FILE}" "$(command dirname ${SSL_KEYSTORE_PATH})"; then
      logInfoResult SUCCESS "done"
    else
      logInfoResult FAILURE "failed"
      exitWithErrorCode CANNOT_CONFIGURE_WORK_DIR_IN_SYSTEM_PROPERTIES
    ...
    logInfo -n "Enabling Jetty-https in ${NEXUS_SYSTEM_PROPERTIES_FILE} overriding ${NEXUS_DEFAULT_CONFIG_FILE}"
    if enableJettyHttpsConfigInNexusCfg "${NEXUS_DEFAULT_CONFIG_FILE}" "${NEXUS_SYSTEM_PROPERTIES_FILE}"; then
      logInfoResult SUCCESS "done"
    else
      logInfoResult FAILURE "failed"
    ...
    logInfo -n "Updating SSL port ${NEXUS_DOCKER_REGISTRY_PORT} in ${NEXUS_SYSTEM_PROPERTIES_FILE}"
    if appendHttpsConnectorPortToNexusCfg "${NEXUS_SYSTEM_PROPERTIES_FILE}" "${NEXUS_DOCKER_REGISTRY_PORT}"; then
      logInfoResult SUCCESS "done"
    else
      logInfoResult FAILURE "failed"
      exitWithErrorCode CANNOT_ENABLE_SSL_IN_NEXUS_SYSTEM_PROPERTIES
    fi
  fi
}
...
addError CANNOT_ENABLE_SSL_IN_NEXUS_SYSTEM_PROPERTIES "Cannot enable SSL in ${NEXUS_SYSTEM_PROPERTIES_FILE}"
