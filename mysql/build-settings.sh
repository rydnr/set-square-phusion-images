...
defineEnvVar UBUNTU_VERSION MANDATORY "The version available in Ubuntu" "$(docker run --rm -it ${BASE_IMAGE_64BIT}:${PARENT_IMAGE_TAG} remote-ubuntu-version mysql-server | sed 's/^.*://g' | sed 's/[^0-9a-zA-Z\._-]//g')";
